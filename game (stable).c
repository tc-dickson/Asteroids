...
void game_setMsPerTick(uint16_t myMsPerTick) {
    msPerTick = myMsPerTick;
}
...
void game_shipControl() {
    ...
    if ((buttons_read() & LEFT_BTN0_MASK) && (buttons_read() & RIGHT_BTN2_MASK)) {
...
//                     while (laster != NULL) {
//                         if (((asteroid->x - laser->x)^2 < (asteroid->radius)^2) &&
//                             ((asteroid->y - laser->y)^2 < (asteroid->radius)^2)) {
...
void game_tick() {
    ...
    switch (currentState) {
        ...
        case play_st:
            if (!enabled) {
            ...
            else if (asteroid_getCount() == 0 && nextLevelCounter >= NEXT_LEVEL_COUNTER_MAX && lives != 0) {
                asteroid_disable();
                nextLevelCounter = 0;
                level++;
                ...
                //check ship for collision, if collision remove life and go to death state
                    //if no lives, go to game over state
