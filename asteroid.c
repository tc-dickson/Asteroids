...
static enum asteroidControl_st_t {
    init_st,
    play_st
} currentState;
...
struct Asteroid* asteroid_addAsteroid(int16_t myX, int16_t myY, int8_t myXVelocity, int8_t myYVelocity, uint8_t myRadius) {
    struct Asteroid *newAsteroid = (struct Asteroid*)malloc(sizeof(struct Asteroid));
    if (newAsteroid) {
        newAsteroid->x = myX;
        newAsteroid->y = myY;
...
void asteroid_collision(struct Asteroid *asteroid) {
    ...
    if (asteroid->radius >= LARGE_ASTEROID_RADIUS) {
        asteroid_addAsteroid(asteroid->x, asteroid->y, asteroid->xVelocity + (rand() % VELOCITY_VARIANCE - VELOCITY_VARIANCE / 2), 
            asteroid->yVelocity + (rand() % VELOCITY_VARIANCE - VELOCITY_VARIANCE / 2), MEDIUM_ASTEROID_RADIUS);
        asteroid_addAsteroid(asteroid->x, asteroid->y, asteroid->xVelocity + (rand() % VELOCITY_VARIANCE - VELOCITY_VARIANCE / 2), 
            asteroid->yVelocity + (rand() % VELOCITY_VARIANCE - VELOCITY_VARIANCE / 2), MEDIUM_ASTEROID_RADIUS);
        asteroid_destroyAsteroid(asteroid);
    }
        ...
        asteroid_addAsteroid(asteroid->x, asteroid->y, asteroid->xVelocity + (rand() % VELOCITY_VARIANCE - VELOCITY_VARIANCE / 2), 
            asteroid->yVelocity + (rand() % VELOCITY_VARIANCE - VELOCITY_VARIANCE / 2), SMALL_ASTEROID_RADIUS);
        asteroid_addAsteroid(asteroid->x, asteroid->y, asteroid->xVelocity + (rand() % VELOCITY_VARIANCE - VELOCITY_VARIANCE / 2), 
            asteroid->yVelocity + (rand() % VELOCITY_VARIANCE - VELOCITY_VARIANCE / 2), SMALL_ASTEROID_RADIUS);
        asteroid_destroyAsteroid(asteroid);
    }
...
void asteroid_testProgram(uint16_t *counter) {
    if (*counter == 0) {
        //struct Asteroid *asteroid = asteroid_addAsteroid(DISPLAY_MID_X, DISPLAY_MID_Y, 3, 3, 24);
    }
    else if (*counter >= 20) {
...
void asteroid_tick() {
    ...
    // They haven't broken me yet, I'm not following the coding standard if it means nothing in the end
}
